...
Southbound emulation & Renode \texttt{antmicro/renode@sha256:a254ed0e}, board \texttt{Stm32F746gDisco} \\
...
series is stationary after the discarded warm-up run: lag-1 autocorrelation $+0.04$,
Spearman correlation with trial index $-0.19$ ($t{=}{-}1.93$, short of significance at
$n{=}100$), and half-campaign means of 1913.3~ms and 1794.0~ms
(Figure~\ref{fig:latency-trial-scatter}). The campaign also induces
...
see a terminal phase. Ninety-seven trials resolve without a retry and span 669--1627~ms
with a coefficient of variation of 0.259; three pay one retry interval, at 2768, 3020 and
3106~ms. That quantization, rather than a heavy-tailed process, is what raises the reported
coefficient of variation to 0.451 and separates the mean (936.8~ms) from the median
(772.0~ms).}
...
\midrule
Enrollment & 916.8 & 878.6 & 1190.8 & 1227.7 & 102.5 & 0.154 & [888.8, 944.8] \\
Heartbeat obs. & 5.7 & 5.6 & 6.9 & 8.5 & 0.7 & 0.182 & [5.5, 5.9] \\
Deployment & 936.8 & 772.0 & 1442.3 & 3020.5 & 265.8 & 0.451 & [852.9, 1020.7] \\
End-to-end & 1853.6 & 1768.4 & 2485.0 & 3883.3 & 283.6 & 0.232 & [1768.4, 1938.8] \\
\bottomrule
\end{tabular}%
}
\end{table}
...
p95 1442.3~ms and p99 3020.5~ms against enrollment's. Excluding the three retried trials its
dispersion is closer to enrollment's than the headline figures suggest (CV 0.259 against
0.154), the residual difference reflecting that enrollment is timed on the protocol exchange
itself whereas deployment is timed on a subsequent status query.}

\begin{figure}[pos=t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/latency_boxplot.png}
...
\begin{figure}[pos=t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/latency_trial_scatter.png}
...
\begin{figure}[pos=t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/latency_cdf.png}
...
Application messages use a 4-byte big-endian length prefix followed by CBOR payloads. Figure~\ref{fig:wire-sizes} summarizes measured wire sizes for representative messages. Heartbeat request and acknowledgment are 6~B each; a complete enrollment round trip totals 265~B of application payload; a minimal deploy round trip (33~B WASM module) totals 124~B. % Removed: At the configured 25~s heartbeat period, steady-state control traffic is $12 \times (3600/25) = 1728$~B/h per device (${\approx}$1.69~KiB/h), excluding TLS record overhead.
...
300~B per exchange and of order 40~KiB/h per device, some $25\times$ the application-layer
...
than by heartbeats: the enrollment payload is 265~B and the handshake carries a
certificate in each direction. The gateway pins TLS~1.2 to the
cipher suites in the Zephyr mbedTLS build; TLS~1.3 would drop the explicit nonce, small
against the framing cost itself.}

\begin{figure}[pos=t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/wire_sizes_bar.png}
...
\begingroup
\color{red}
The evaluated Zephyr/WAMR/TLS/CBOR firmware fits the STM32F746NG internal-memory budget: the flash-resident \texttt{zephyr.bin} image is 411{,}272~B ($\approx$402~KiB), corresponding to text plus initialized data, and the static RAM allocation is 273{,}503~B ($\approx$267~KiB), corresponding to data (9{,}384~B) plus BSS (264{,}119~B). \revD{Mutual authentication costs 4{,}704~B of flash and 3{,}265~B of RAM -- the curve and X.509 code is already linked in for the TLS handshake itself. The linker's 256~KiB SRAM1 region, which is separate from the 64~KiB DTCM, is left 99.3\% full, and that is what now bounds further additions: raising the mbedTLS heap to 64~KiB overflows it at link time.}\footnote{The \texttt{zephyr.elf} file is 8{,}095{,}176~B (7.72~MiB), but only because it carries ELF metadata and debug symbols for the Renode \texttt{sysbus LoadELF} workflow; it is not the image written to device flash.} The benchmark WASM module remains 33~B. The architectural point supported by these measurements is therefore that embedded endpoints run no kubelet, container runtime, or Linux userspace while retaining an embedded-class memory footprint.
\endgroup
...
Figure~\ref{fig:pod-resources} reports mean Kubernetes pod usage ($n{=}5$ samples, one TLS-connected device, sampled while the trial campaign was running). {\color{red}The metric is the container working set reported by \texttt{metrics-server}, cross-checked against cgroup~v2 \texttt{memory.current} and the anonymous resident set in \texttt{memory.stat}. The two agree closely for the gateway and the controller; the API server's working set fluctuates between samples while its \texttt{memory.current} stays steady, so the sampled mean understates it.} The gateway averaged 2.4~millicores CPU and 4.0~MiB RAM (\texttt{memory.current} 5.25~MB, anonymous 3.43~MB), the API server averaged 181.4~millicores and 26.0~MiB under campaign load (\texttt{memory.current} 31.7~MB), and the application controller averaged 79.6~millicores and 12.0~MiB (\texttt{memory.current} 13.4~MB). %Removed (minore): Registering 50 synthetic boards through the gateway HTTP registry left gateway resident memory unchanged (\texttt{memory.current} identical before and after), indicating lightweight per-device mediation state at the fog layer.
...
\begin{figure}[pos=t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/pod_resources_bar.png}
...
\midrule
Enrollment & gateway & 0.000527 & 0.000446 & \revD{0.000424--0.000786} & \revD{[0.000431, 0.000666]} \\
Enrollment & api-server & 0.211061 & 0.146935 & \revD{0.117500--0.344147} & \revD{[0.129463, 0.295833]} \\
Enrollment & app-controller & 0.109378 & 0.109339 & \revD{0.100157--0.116024} & \revD{[0.104368, 0.114173]} \\
Heartbeat & gateway & 0.000739 & 0.000648 & \revD{0.000432--0.001431} & \revD{[0.000496, 0.001095]} \\
Heartbeat & api-server & 0.035672 & 0.000000 & \revD{0.000000--0.096947} & \revD{[0.000000, 0.074451]} \\
Heartbeat & app-controller & 0.028725 & 0.028743 & \revD{0.021564--0.032722} & \revD{[0.025062, 0.031752]} \\
Deployment & gateway & 0.010171 & 0.010998 & \revD{0.006330--0.013636} & \revD{[0.007791, 0.012254]} \\
Deployment & api-server & 0.790809 & 0.881200 & \revD{0.437644--1.108854} & \revD{[0.553002, 1.037498]} \\
Deployment & app-controller & 0.129507 & 0.144385 & \revD{0.044876--0.172308} & \revD{[0.084680, 0.160578]} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
differently: the gateway averages 2.4~millicores against the API server's 181.4, so the
...
\midrule
Edge runtime & kubelet + container runtime + OS & Zephyr + WAMR ({\color{red}402~KiB flash, 267~KiB RAM}) \\
...
{\color{red}Steady heartbeat traffic} & CRI/container probes & {\color{red}1728~B/h per device (application layer)}\revD{, ${\approx}40$~KiB/h on the wire} \\

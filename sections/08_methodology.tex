...
\revD{Deployment carries a heavy upper tail (CV 0.43, maximum 3000.8~ms against a median of
574.1~ms). The tail reflects how the harness observes completion rather than the
orchestration path: it polls the Application resource on a fixed two-second cycle, so a
trial whose evidence appears just after a poll is charged a full cycle. Seventy trials fall
in a 487--598~ms band and twenty-eight more between 603 and 1112~ms; only two exceed that,
the largest at 3000.8~ms, one polling cycle beyond the rest. Deployment latency is
therefore an upper bound inclusive of observation granularity.}
...
\revD{Figure~\ref{fig:latency-cdf} shows that deployment contributes most of the tail mass: its
p95 (1021.8~ms) and p99 (1145.2~ms) exceed enrollment's, with a larger coefficient of
variation (0.428 against 0.164). Given the polling granularity, this separates the stages
by how they are observed: enrollment is timed on the protocol exchange, deployment on the
first poll that sees converged status.}

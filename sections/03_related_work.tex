...
\revD{The closest prior art is not an orchestrator at all but the device twin, or shadow,
pattern: a cloud-resident document holding desired and reported state for a device that is
too constrained or too intermittently connected to be queried directly, reconciled by a
managed service as the device reconnects. AWS IoT Greengrass~\cite{aws_greengrass} and
Azure IoT Edge~\cite{azure_iot_edge} have offered this in production for a decade, and
LwM2M~\cite{lwm2m} provides a standardized object model with comparable semantics. The
evidence model used here is deliberately the same pattern, and we claim no novelty in the
idea of separating desired from reported state. What differs is where the reconciliation
point lives. In twin-based platforms the authoritative state sits in a proprietary managed
service, and integration with cluster tooling happens afterwards through bridges or
exports. Here desired and reported state are Kubernetes resources from the outset, so
status ownership, admission control, RBAC, watches, and the controller pattern apply to
constrained devices without a translation layer, and a placement or sustainability policy
can be written as an ordinary controller against the same objects. That is an
integration-level claim about where the boundary is drawn, not a new reconciliation
mechanism.}

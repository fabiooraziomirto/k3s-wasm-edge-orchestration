...
%Removed (minore): The problem management is structured on three pillars, standing on the three level in which framework acts: 
\revD{The problem is structured around three pillars, corresponding to the three levels at which the framework acts:}
...
R2: Identity binding & A protected channel is insufficient unless the endpoint is bound to the expected platform identity. & Both ends authenticate with fleet-issued certificates, and the device signs a single-use nonce before its session is associated with a Device resource. & Enrollment success, identity match, refusal of a device that cannot sign the nonce. \\

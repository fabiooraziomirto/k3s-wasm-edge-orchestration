...
gateway propagates the outcome into control-plane status (Figure~\ref{fig:deployment-sequence}).}
...
resource; heartbeats then refresh that association. Deployment runs the complementary
direction, and the returned \texttt{DeployResult} is treated as execution evidence rather
than as a transport acknowledgment: application status converges only once that evidence is
propagated.}

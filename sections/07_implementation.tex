...
\revD{The security scope of the evaluated prototype should be stated explicitly, because
the architecture and the measured configuration do not yet coincide. The design assumes a
mutually authenticated southbound channel, in which the gateway verifies a device
certificate issued by the platform authority and the device verifies the gateway. The
prototype measured here does not implement that assumption: the gateway terminates TLS 1.2
without requesting a client certificate, and the firmware does not verify the gateway
certificate, so the session protects confidentiality and integrity against a passive
observer but authenticates neither endpoint against an active on-path attacker. Identity
binding is therefore established one layer higher, and is the property the evaluation
actually exercises: the gateway compares the public-key material carried in the enrollment
record against the key registered in the \texttt{Device} resource and refuses the
association when the two disagree. Three further limits follow. The expected key is
provisioned out of band when the \texttt{Device} resource is created, so first contact is
trust-on-first-configuration rather than attested. Heartbeats carry no nonce or counter,
so replay is prevented by the TLS record layer within a session but not across sessions.
The dispatched WASM module is accepted on the authority of the control plane, without an
artifact-level integrity check on the device. Closing these gaps is the first hardening
step for the platform rather than a research question: workload identity
attestation~\cite{spiffe}, signed artifact metadata~\cite{tuf}, and the update-security
architecture of RFC~9019~\cite{rfc9019} provide established building blocks.}
...
Larger WASM modules, richer WASI support, and application-specific input/output channels can be added above the same deployment and evidence path without changing the control ownership model.
\endgroup

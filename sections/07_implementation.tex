...
\revD{The southbound channel is mutually authenticated. Each device holds an ECDSA P-256 key
pair and a certificate issued by a fleet authority; the gateway requires that certificate and
the firmware verifies the gateway against the same authority. A device is admitted only when
two independent checks agree: the key announced at enrollment is the key the peer
authenticated the transport with, and the device signs a single-use gateway nonce with the
matching private key. Since a public key is not a secret, the second check separates holding
an identity from having observed one; since the nonce is consumed whether or not the
signature verifies, a captured response cannot be replayed. Dispatched modules carry a
SHA-256 digest the firmware recomputes and refuses to instantiate on mismatch. The mechanism
is exercised, not only implemented: a certificate the authority did not issue, an absent
certificate, and a nonce signed with the wrong key are each refused, the last leaving no
association recorded. The signature costs 79~ms on the emulated target, where enrollment
completes 111--185~ms after the TLS session opens. Revocation, a second issuing authority,
TLS~1.3, and a digest bound to a publisher's signature remain outstanding; workload identity
attestation~\cite{spiffe} and signed artifact metadata~\cite{tuf,rfc9019} address the last.}
...
against the 327{,}680~B of internal SRAM on the STM32F746NG, the 273{,}503~B static
allocation reported in Section~\ref{sec:evaluation} leaves roughly 53~KiB for the WAMR
heap, the stacks and the module itself. Establishing the largest module the path can carry,
and whether the transfer needs chunking, requires measurement not attempted here.}
\endgroup
